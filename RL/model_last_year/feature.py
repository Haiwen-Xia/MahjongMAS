...
class FeatureAgent(MahjongGBAgent):
    ...
    def __init__(self, seatWind):
        ...
        # self.obs[self.OFFSET_OBS['REMAIN0' : 'REMAIN3' + 1]][0 : 21] = 1
    ...
    def request2obs(self, request):
        ...
        if t[0] == 'Draw':
            ...
            # 减少牌墙数量
            self.tileWall[0] -= 1
            # self.obs[self.OFFSET_OBS['REMAIN0']][self.tileWall[0]] = 0
        ...
        if t[2] == 'Draw':
            ...
            # 减少玩家 p 的牌墙数量
            self.tileWall[p] -= 1
            # self.obs[self.OFFSET_OBS['REMAIN%d' % p]][self.tileWall[p]] = 0
        ...
        if t[2] == 'Play':
            ...
            self.curTile = t[3]
            # 更新打出的牌数量
            # self.obs[self.OFFSET_OBS['PLAY%d' % p]][self.curTile] = 1
        ...
        if t[2] == 'Chi':
            ...
            num = int(tile[1])
            # 更新吃牌的记录
            # self.obs[self.OFFSET_OBS['CHI%d' % p]][self.OFFSET_TILE[color + '%d' % num]] = 1
        ...
        if t[2] == 'UnChi':
            ...
            self.packs[p].pop()
            self.shownTiles[self.curTile] += 1
            # self.obs[self.OFFSET_OBS['CHI%d' % p]][self.OFFSET_TILE[color + '%d' % num]] = 0
    ...
    def _obs(self):
        ...
        for p in range(4):
            for operate in self.packs[p]:
                ...
                if tile == 'CHI':
                    for i in range(4):
                        if self.obs[self.OFFSET_OBS[act + '%d' %p] + pos * 4 + i][self.OFFSET_TILE[tile]] == 0:
                            self.obs[self.OFFSET_OBS[act + '%d' %p] + pos * 4 + i][self.OFFSET_TILE[tile]] = 1;
                            break;
                else:
                    self.obs[self.OFFSET_OBS[act + '%d' % p] + pos][self.OFFSET_TILE[tile]] = 1
                    
        if self.AnGang:
            self.obs[self.OFFSET_OBS['ANGANG']][self.OFFSET_TILE[self.AnGangTile]] = 1
        ...
        # 添加最后的4个维度[剩余牌数]
        for p in range(4):
            for j in range(self.tileWall[p]):
                self.obs[self.OFFSET_OBS['REMAIN%d' % p]][j] = 1
