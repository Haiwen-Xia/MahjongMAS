...
class SimpleSpace:
    """一个简单的类，用于描述数据空间的形状和类型，替代 gym.spaces.Box。"""
    def __init__(self, shape, dtype=np.float32):
        self.shape = shape
        self.dtype = dtype

class DictSpace:
    """一个简单的类，用于描述字典形式的数据空间，替代 gym.spaces.Dict。"""
    def __init__(self, spaces_dict):
        self.spaces = spaces_dict
        # 我们可以让它也有一个 shape 属性，尽管对于字典来说意义不大
        # 但有些框架可能会访问它。这里我们将其设为 None。
        self.shape = None 

class DiscreteSpace(SimpleSpace):
    def __init__(self, n):
        super().__init__(shape=(), dtype=np.int64) # 离散动作通常是标量
        self.n = n
...
class MahjongGBEnv():
    ...
    def __init__(self, config):
        ...
        if issubclass(self.agentclz, FeatureAgent):
            self.observation_space = [ # 为每个智能体定义
                DictSpace({
                    'observation': SimpleSpace(shape=(187, 4, 9), dtype=np.float32),
                    'action_mask': SimpleSpace(shape=(ACT_SIZE,), dtype=np.int8)
                }) for _ in range(self.n_agents)
            ]
            
            # 定义全局观测空间 (share_observation_space)
            # 假设它是一个 256 维的向量
            self.share_observation_space = [
                SimpleSpace(shape=(256,), dtype=np.float32) for _ in range(self.n_agents)
            ]

            # 定义动作空间 (通常是离散的)
            self.action_space = [DiscreteSpace(ACT_SIZE) for _ in range(self.n_agents)]


        elif isinstance(self.agentclz, FeatureAgentTimeSeries):
            raise NotImplementedError("FeatureAgentTimeSeries is not supported yet!")
        else:
            raise NotImplementedError("Only FeatureAgent is supported for now!")
    ...
    def reset(self, prevalentWind = -1, tileWall = '', use_random_seed=None):
        ...
        self.shownTiles = defaultdict(int)        # Deal cards
        self._deal()
        return self._obs(), self._get_global_obs()
    ...
    def step(self, action_dict):
        try:
            ...
            self.done = True
        return self._obs(), self._reward(), self._done(), self._get_global_obs()
    ...
    def _get_global_obs(self):
        ...
        前接 deal
        """
        self.global_obs = np.zeros((16, 36), dtype=np.float32) 
        ...
        for player in range(4):
            ...
            for tile in d:
                self.global_obs[player * 4: player * 4 + d[tile], OFFSET_TILE[tile]] = 1

        return self.global_obs.reshape((16, 4, 9)).astype(np.float32)
