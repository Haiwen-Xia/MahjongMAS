...
if __name__ == '__main__':
    ...
    while True:
        ...
        if t[0] == '0': 
            ...
            if action_type == 'DRAW':
                # 其他玩家摸牌
                obs_to_memorize = agent.request2obs('Player %d Draw' % p) # 记录信息
                # MODIFY: 让模型记住这个玩家摸过了牌
                obs2response(model, obs_to_memorize) # 让模型决策下一步的动作
                ...
                if p == seatWind and angang:
                    ...
                    # MODIFY: 让模型记住这个玩家暗杠杠过了牌
                    obs = agent.request2obs('Player %d AnGang %s' % (p, angang))
                    obs2response(model, obs)
                    angang = None # 重置angang标记

                elif zimo: # 如果该玩家是摸牌后杠, 则为暗杠

                    # MODIFY: 让模型记住这个玩家暗杠杠过了牌
                    obs = agent.request2obs('Player %d AnGang' % p)
                    obs2response(model, obs)

                else: # 否则为明杠（大明杠, 杠了别人打的牌） [*]

                    # MODIFY: 其他人杠牌, 模型也要记住
                    obs = agent.request2obs('Player %d Gang' % p)
                    obs2response(model, obs)
                ...
                if action_type == 'CHI':
                    ...
                    # MODIFY: 其他人吃牌, 模型也要记住
                    obs = agent.request2obs('Player %d Chi %s' % (p, t[3]))
                    obs2response(model, obs)
                    ...
                    # MODIFY: 其他人碰牌, 模型也要记住
                    obs = agent.request2obs('Player %d Peng' % p)
                    obs2response(model, obs)
                ...
                if p == seatWind:
                    ...
                    if res_parts[0] == 'Hu': # 和牌
                        ...
                        # 因此我们要将状态恢复到未碰/吃的状态
                        obs_for_forget = agent.request2obs('Player %d Un%s' % (seatWind, response)) # 例如: Player 0 UnPeng / Player 0 UnChi T2

                        # 5. 把遗忘这件事告诉模型
                        obs2response(model, obs_for_forget)
