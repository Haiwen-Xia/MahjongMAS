...
class ModelPoolServer:
    ...
    def __init__(self, capacity, name):
        ...
        metadata_size = 1024 # 为每个序列化后的元数据条目预留的字节大小 (需要足够大以容纳 cPickle.dumps(metadata))
    ...
    def push(self, state_dict, metadata=None):
        ...
        # 需要确保序列化后的元数据不超过预设的 metadata_size
        serialized_metadata = cPickle.dumps(current_metadata)
        if len(serialized_metadata) > self.shared_model_list.format.split('s')[0]: # 简单检查大小
             # print(f"Warning: Serialized metadata for model {self.n} is too large for the slot!")
             # 实际应用中应处理此错误，例如增加 metadata_size 或截断/选择性序列化
             # 这里简单截断，可能导致客户端 cPickle.loads 失败
             serialized_metadata = serialized_metadata[:self.shared_model_list.format.split('s')[0]]
